...
contract TokenToken is ERC20 {
     ...
     */
    constructor() public ERC20('PolyToken', 'Token') {
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "PolyTokenToken::transferOperator: new operator is the zero address");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Max deposit fee: 10%.
    uint16 public SHARE_VAULT_DEPOSIT = 0;
    
    // Initial emission rate: 0.6 Token per block.
    uint256 public constant INITIAL_EMISSION_RATE = 0.6 ether;
    // Max Token Supply 2 500 000
    uint256 public constant MAX_SUPPLY_CAP = 2500000 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed TokenPerBlock);
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (IERC20(Token).totalSupply() + totalLockedUpRewards  >= MAX_SUPPLY_CAP) {
            return 0;
        }
    ...
    // View function to see if user can harvest Token's.
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                pool.lpToken.safeTransfer(feeAddress, depositFee.mul(100 - SHARE_VAULT_DEPOSIT).div(100));
                if(SHARE_VAULT_DEPOSIT > 0) {
                    pool.lpToken.safeTransfer(vaultAddress, depositFee.mul(SHARE_VAULT_DEPOSIT).div(100));
                }
    ...
    function updateVaultDepositShare(uint16 _shareVaultDeposit) external onlyOwner {
        require(_shareVaultDeposit >= 0, "updateVaultDepositShare: invalid vault deposit share.");
        require(_shareVaultDeposit <= 100, "updateVaultDepositShare: invalid vault deposit share.");
        SHARE_VAULT_DEPOSIT = _shareVaultDeposit;
    }
}